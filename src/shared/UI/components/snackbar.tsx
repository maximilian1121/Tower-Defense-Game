...
const SnackbarContext = createContext<
    ((message: string, options?: SnackbarOptions) => void) | undefined
>(undefined);
...
function AnimatedSnack({
    ...
    useEffect(() => {
        ...
        const connection = animationValue
            .GetPropertyChangedSignal("Value")
            .Connect(() => {
                setProgress(animationValue.Value);
            });

        const entryTween = TweenService.Create(animationValue, tweenInfo, {
            Value: 1,
        });
        entryTween.Play();
        ...
        const delay = task.delay(waitTime, () => {
            const exitTween = TweenService.Create(animationValue, tweenInfo, {
                Value: 0,
            });
            exitTween.Play();
    ...
    const animatedPosition = progress.map(
        (p: number) => new UDim2(1 - p, 0, 0, 0),
    );
